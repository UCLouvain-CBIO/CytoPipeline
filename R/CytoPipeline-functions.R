...
execute <- function(x,
    ...
    for (s in seq_along(x@scaleTransformProcessingQueue)) {
        ...
        if (cacheResourceName %in% BiocFileCache::bfcinfo(bfc)$rname) {
            ...
            genericMeta <- 
                data.frame(list(rid = names(cacheResourceFile),
    ...
    if (inherits(res, "transformList")) {
        ...
        if (x@saveScaleTransform) {
            if (!length(x@scaleTransformFile))
                stop("saving scale tranformations require a ",
        ...
        for (s in seq_along(x@flowFramesPreProcessingQueue)) {
            ...
            if (cacheResourceName %in% BiocFileCache::bfcinfo(bfc)$rname) {
                ...
                BiocFileCache::bfcmeta(
                    bfc, name = "preprocessing", append = TRUE) <-
                    preprocessingMeta
    ...
    function(x,
        ...
        if (!is.null(sampleFile)) {
            ...
            if (length(indexInCache) > 1) {
                stop("More than one line in cache corresponding to object ",
                     "name and sample file => unexpected inconsistency")
            }
            
            if (length(indexInCache) == 0) {
                stop("Combination (sample file, object name) ",
                     "not found in cache")
            }
        } else {
    ...
    function(x,
        ...
        if (!is.null(sampleFile)) {
            ...
            if (length(indexesInCache) == 0) {
                stop("Combination ('", whichQueue,"'", sampleFile, 
                     ") not found in cache")
            }
        } else {
            if (!("fcsfile" %in% colnames(cacheInfo))) {
    ...
    function(x,
        ...
        if (whichQueue == "pre-processing") {
            if (is.null(sampleFile) && length(x@sampleFiles) > 0) {
                message("no sample file passed as argument ",
