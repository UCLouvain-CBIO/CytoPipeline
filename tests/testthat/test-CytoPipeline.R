...
test_that("CytoPipeline with reading scale transfo only raises no error", {
  expect_error({
    ...
    suppressWarnings(execute(pipL, rmCache = TRUE, 
                             path = outputDir))
...
test_that("Check consistency with cache works", {
  ...
  expect_error(suppressWarnings(execute(pipL5, rmCache = TRUE,
                                        path = outputDir)), NA)
  ...
  expect_error(execute(pipL5, rmCache = FALSE, 
                       path = outputDir), NA)
  res <- checkCytoPipelineConsistencyWithCache(pipL5,
                                               path = outputDir)
  expect_equal(res$isConsistent, TRUE)
  ...
  expect_error(suppressWarnings(execute(pipL5, rmCache = FALSE,
                                        path = outputDir)), NA)
  res <- checkCytoPipelineConsistencyWithCache(pipL5,
                                               path = outputDir)
  expect_equal(res$isConsistent, TRUE)
...
test_that("plotCytoPipelineProcessingQueue works", {
  ...
  expect_error(suppressWarnings(execute(pipL6, rmCache = TRUE,
                                        path = outputDir)), NA)
  ...
  expect_error(execute(pipL6, rmCache = FALSE,
                       path = outputDir), regexp = "inconsistent")
})
